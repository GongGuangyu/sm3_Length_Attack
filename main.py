...
def IterFunction(message):
    ...
    for i in range(n):
        ...
        v.append(temp)
    return v[n]
def IterFunction_length_attack(message,vi):
    n = int(len(message) / 512)
    v = []
...
def Rho_Attack(n):
    ...
    for i in range(0xffff):
        ...
        if(SM3(a)[:int(n/4)]in res):
            ...
            return;
    print('Failed')
def Length_Attack(msg1,msg2):
    vi_1=int(SM3(msg1),16)#得到初始的VI，替代原先的固定VI

    msg1_appding=fillFunction(msg1)#原始消息填充后的消息

    length=len(msg1_appding)#原始消息填充后消息的长度，要加到后面的长度上的

    msg2_appding=fillFunction(msg2)#扩展消息的填充

    msg2_appding1=hex(int(msg2_appding,2)+length)[2:]#扩展后总的消息长度，包括原始消息及其填充的长度
    a=''
    for x in msg2_appding1:
        a=a+'{:04b}'.format(int(x,16))
    res=IterFunction_length_attack(a,vi_1)
    result_1 = hex(int(res, 2)) #这个结果是把hash（msg1）的结果当做IV然后去hash（msg2）也就是长度扩展攻击的结果
    print(result_1)

    d=((len(hex(msg2)) - 2) * 4)
    f=int(msg1_appding,2)
    msg3=(f<<d)+msg2
    result_2=SM3(msg3)
    print(result_2)#result2是msg1+apdding+msg2的结果
...
if __name__ == '__main__':
    ...
    Length_Attack(0x616263,0x123456)
    time_end = time.time()
